...
class Schedule:
    ...
    def to_dataframe(self):
        df = pd.DataFrame(
            columns=['group', 'lesson_num', 'lesson', 'weeks', 'weekday', 'teachers', 'time_start', 'time_end', 'type',
                     'room', 'campus', 'subgroup'])
        ...
        for lesson in self.lessons:
            if type(lesson) is not LessonEmpty:
                ...
                lesson_campus = lesson_campus.value if lesson_campus is not None else np.nan
                ...
                df.loc[len(df)] = [self.group, lesson.num, lesson.name, weeks, lesson.weekday.value[1], teachers,
                                   lesson.time_start, lesson.time_end,
                                   lesson_type, lesson_room, lesson_campus, lesson.subgroup if lesson.subgroup else np.nan]
