...
class CollegeParser(ScheduleParser):
    ...
    def __get_lessons(self, names, types, teachers, rooms, times):
        ...
        lesson_weeks = self.__formatter.get_weeks(names, None, self.max_weeks)
    ...
    def __read_group_for_semester(
        ...
        """
        # Название группы
        cell_group_value = sheet.cell(group_name_row_num, discipline_col_num).value
        ...
        for day_num in cell_range:
            ...
            for lesson_range in cell_range[day_num]:
                ...
                lesson_types = "пр"
                lesson_teachers = sheet.cell(string_index, discipline_col_num).value
                lesson_rooms = sheet.cell(string_index, discipline_col_num + 1).value

                lesson_rooms = (
                    str(int(lesson_rooms))
                    if isinstance(lesson_rooms, float)
                    else str(lesson_rooms)
                )
    ...
    def __get_column_range(
        self, xlsx_sheet, group_name_cell, group_name_row, group_name_row_index
    ):
        ...
        day_num_val, lesson_num_val, lesson_time_start_val, lesson_time_end_val = (
            0,
            0,
            0,
            0,
