...
class Formatter(metaclass=ABCMeta):
    ...
    def get_weeks(
        ...
        Args:
            ...
            is_even: True, если предмет проводится только по чётным неделям и False, если по нечётным. Если None,
                то чётность недели не важна (проводится на всех неделях). По умолчанию None.

            max_weeks: Максимальное количество недель в семестре. Необходимо для тех
                случаев, когда у предмета список недель не указан, т.е. когда предмет проводится на всех неделях
                семестра. По умолчанию None.
