...
class CollegeFormatter(Formatter):
    def __split_lessons(self, lesson):
        ...
        if len(lessons_weeks_match) == 2:
            ...
            result.append(
                lesson[
                    lessons_weeks_match[0].span()[0] : lessons_weeks_match[1].span()[0]
                ].strip()
            )
    ...
    def get_teachers(self, teachers_names: str):
        teachers = re.findall(
            r"(?:[а-яА-Я]+\s{1,3}[а-яА-Я]\.\s?[а-яА-Я](?:\.|\s|\b|$))", teachers_names
        )
    ...
    def get_lessons(self, lesson: str) -> list:
        ...
        for i in range(len(result)):
            ...
            lesson_preffix = (
                lesson_preffix.group(0) + " " if lesson_preffix is not None else ""
            )
